...
AND c.id NOT IN (SELECT c.id FROM contracts AS c INNER JOIN authentication_contracts AS ac ON ac.contract_id = c.id INNER JOIN contract_events AS ce ON ce.contract_id = c.id WHERE ce.description LIKE '%Qtde contas suspensas%')
