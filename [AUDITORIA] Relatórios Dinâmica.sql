...
INNER JOIN 
	bank_accounts AS bank ON fatr.bank_account_id = bank.id
LEFT JOIN 
	contracts AS c ON fatr.client_id = c.client_id
WHERE 
	bank.id = 75
	AND fatr.complement LIKE "%Din%"
