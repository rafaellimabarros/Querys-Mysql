SELECT
	...
	(SELECT serv.title FROM service_products AS serv WHERE serv.id = ci.service_product_id) AS servico,
	ci.unit_amount AS valor_servico,
...
INNER JOIN 
	contract_items AS ci ON ca.contract_id = ci.contract_id
LEFT JOIN
...
WHERE
	ca.activation_date BETWEEN '2022-02-01' AND '2022-02-28' AND c.company_place_id != 3 AND ci.service_product_id IN (2762, 2761) /*  AND camp.campaign_id = 2 ----- AND pl.service_product_id = 2852*/
GROUP BY
	c.id
ORDER BY
	ca.activation_date
