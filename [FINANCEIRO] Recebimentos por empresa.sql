SELECT
	...
	p.city AS cidade,
	(SELECT emp.description FROM companies_places as emp WHERE fatr.company_place_id = emp.id) AS empresa_contrato,
	...
	(SELECT bank.description FROM bank_accounts AS bank WHERE bank.id = fatr.bank_account_id) AS conta_liq,
...
WHERE 
	fatr.receipt_date BETWEEN '2021-06-01' AND '2021-06-31' AND fat.financial_collection_type_id IS NOT NULL AND fatr.company_place_id = 2 
ORDER BY
